...
for ss = 1:Ns
    ...
    for kk = K-params.ARO:-1:2
        ...
        for jj = 1:Np
            ...
            [A, Q] = construct_transmats(flags, params, filt_pts(jj).nonlin_samp(1:P,kk-1), filt_pts(jj).nonlin_samp(params.ARO+1,kk-1));
            [pred_mn, pred_vr] = kf_predict(smooth_pts(ss).back_lin_mn(1:P,kk), smooth_pts(ss).back_lin_vr(1:P,1:P,kk), inv(A), (A\Q/A'));
            assert(isposdef(pred_vr));
            assert(isposdef(filt_pts(jj).lin_vr(1:P,1:P,kk-1)+pred_vr));
            
            % Linear probability bit
            sampling_weights(jj) = sampling_weights(jj) ...
                + log( mvnpdf(filt_pts(jj).lin_mn(1:P,kk-1)', pred_mn', filt_pts(jj).lin_vr(1:P,1:P,kk-1)+pred_vr) )...
                - log( abs(det(A)) );
        ...
        [A, Q] = construct_transmats(flags, params, filt_pts(samp_ind).nonlin_samp(1:P,kk-1), filt_pts(samp_ind).nonlin_samp(params.ARO+1,kk-1));
        [pred_mn, pred_vr] = kf_predict(smooth_pts(ss).back_lin_mn(1:P,kk), smooth_pts(ss).back_lin_vr(1:P,1:P,kk), inv(A), A\Q/A');
        H = [zeros(1, P-1) 1];
        R = params.noise_vr;
        [smooth_pts(ss).back_lin_mn(1:P,kk-1), smooth_pts(ss).back_lin_vr(1:P,1:P,kk-1)] = kf_update_1D(pred_mn, pred_vr, observs(kk-P+1), H, R);
